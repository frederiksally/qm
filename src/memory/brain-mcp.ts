...
export function createBrainMcp(opts: {
  mcpUrl: string;
  ...
  ): Promise<{ ok: boolean; status: number; body: string; headers?: BrainResponse["headers"] }> => {
    const abort = new AbortController();
    let timer: ReturnType<typeof setTimeout> | undefined;
    try {
      return await Promise.race([
        fetchImpl(url, { ...init, signal: abort.signal }).then(async (response) => ({
          ok: response.ok,
        ...
        new Promise<never>((_, reject) => {
          timer = setTimeout(() => {
            abort.abort();
            reject(new Error(`brain request timed out after ${requestTimeoutMs}ms`));
          }, requestTimeoutMs);
        }),
      ]);
    } finally {
  ...
  return {
    ...
    async mintToken(clientId, clientSecret) {
      ...
      const body = (safeJson(res.body) ?? {}) as { access_token?: unknown; expires_in?: unknown };
    ...
    async call(accessToken, name, args) {
      ...
      if (!res.ok) {
        const failure =
          res.status === 404 ? parseMcpEnvelope(res.body, res.headers?.get("content-type"), id) : null;
      ...
      const parsed = parseMcpEnvelope(res.body, res.headers?.get("content-type"), id);
