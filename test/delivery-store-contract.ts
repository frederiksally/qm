...
export async function exerciseDeliveryStore(store: DeliveryStore): Promise<void> {
  ...
  await store.completeStreamByKey("run:r-3", "171.004");
  assert.equal((await store.get(late.id))?.destination.stream, undefined);
  assert.equal((await store.get(late.id))?.destination.preserveMessage, true);
  ...
  assert.equal((await store.get(late.id))?.destination.stream, undefined, "delivered stream copies are not patched");
